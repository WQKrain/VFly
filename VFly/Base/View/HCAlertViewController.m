...
@property (copy, nonatomic) void(^actionHandler)(HCAlertAction *action);
...
+ (instancetype)actionWithTitle:(NSString *)title handler:(void (^)(HCAlertAction *action))handler {
    HCAlertAction *instance = [HCAlertAction new];
...
@interface HCAlertViewController ()
{
    UIView *_shadowView;
    UIView *_contentView;
...
@implementation HCAlertViewController

+ (instancetype)alertControllerWithTitle:(NSString *)title message:(NSString *)message {
    
    HCAlertViewController *instance = [HCAlertViewController new];
...
- (void)defaultSetting {
    // 上 左 底 右
    _contentMargin = UIEdgeInsetsMake(10, 20, 0, 20);
    _contentViewWidth = 245;
    _buttonHeight = 45;
    _firstDisplay = YES;
...
- (void)creatAllSeparatorLine {
    ...
    for (int i=0; i<linesAmount; i++) {
        ...
        separatorLine.backgroundColor = kGrayColor;
...
- (void)updateMessageLabelFrame {
    ...
    CGFloat messageY = self.title.length ? CGRectGetMaxY(_titleLabel.frame) + 10 : _contentMargin.top + 10;
...
- (void)updateAllButtonsFrame {
    ...
    for (int i=0; i<self.actions.count; i++) {
        ...
        // 首个按钮与messageLabel的纵向间距加大
        btn.frame = CGRectMake(buttonX, buttonY + 10, buttonWidth, _buttonHeight);
        
        if (1 == self.actions.count)
        {
            [btn setTitleColor:kMainColor forState:UIControlStateNormal];
        }
        else if (2 == self.actions.count)
        {
            if (1 == i) {
                [btn setTitleColor:kMainColor forState:UIControlStateNormal];
            }
...
- (void)updateShadowAndContentViewFrame {
    ...
    frame.size.height = firstButtonY+allButtonHeight + 10;
...
- (void)didClickButton:(UIButton *)sender {
    HCAlertAction *action = self.actions[sender.tag-10];
...
{
    if (self.needDismissAnimation == NO)
    {
