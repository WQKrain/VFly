...
@interface VFTwoColumnsSheetPickerView()<UIPickerViewDelegate, UIPickerViewDataSource>
...
@implementation VFTwoColumnsSheetPickerView
...
+ (instancetype)VFTwoColumnsSheetPickerViewWithTitle:(NSArray *)title rightArr:(NSArray *)rightArr andHeadTitle:(NSString *)headTitle Andcall:(twoColumnsSheetPickerViewBlock)callBack{
    VFTwoColumnsSheetPickerView *pickerView = [[VFTwoColumnsSheetPickerView alloc]initWithFrame:[UIScreen mainScreen].bounds andTitle:title rightArr:rightArr andHeadTitle:headTitle];
    pickerView.callBack = callBack;
    return pickerView;
}

- (instancetype)initWithFrame:(CGRect)frame andTitle:(NSArray*)title rightArr:(NSArray *)rightArr andHeadTitle:(NSString *)headTitle
{
    self = [super initWithFrame:frame];
    if (self) {
        self.dataArray = title;
        self.rightArray = rightArr;
        _headTitle = headTitle;
        _backString = self.dataArray[0];
        _rightString = self.rightArray[0];
        [self setupUI];
    }
    return self;
}
...
-(void)setupUI
{
    ...
    //  标题
    UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(SCREEN_SIZE.width/2-75, 5, 150, 40)];
    ...
    cancelButton.frame = CGRectMake(2, 5, KScreenWidth*0.25, 40);
    ...
    doneButton.frame = CGRectMake(KScreenWidth-KScreenWidth*0.25-2, 5, KScreenWidth*0.25, 40);
...
- (UIPickerView *)pickerView
{
    if (!_pickerView) {
        //选择器
        _pickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(5,42, SCREEN_SIZE.width-10, 230)];
...
- (void)clicked:(UIButton *)sender
{
    if ([sender isEqual:self.cancelButton]) {
        ...
        if (self.callBack) {
            self.callBack(self,_backString,_rightString);
        }
        [self dismissPicker];
    }
}
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    if (0 == component) {
        ...
        _rightString = self.rightArray[row];
    }
}
...
- (void)show
{
    [UIView animateWithDuration:0.5f delay:0 usingSpringWithDamping:0.9f initialSpringVelocity:0.7f options:UIViewAnimationOptionCurveEaseInOut | UIViewAnimationOptionBeginFromCurrentState | UIViewAnimationOptionLayoutSubviews animations:^{
