//
//  VFContinueRentCarViewController.m
//  LuxuryCar
//
...
@interface VFContinueRentCarViewController ()<TXTimeDelegate,VFRenewalChooseRentDaysDeleaget>
...
@implementation VFContinueRentCarViewController
...
- (void)loadData{
    ...
    [HttpManage getOrderMessageWithOrderID:self.orderID With:^(NSDictionary *dic) {
        ...
        _detailObj = [[OrderDetailModel alloc]initWithDic:dic];
         [_carImage sd_setImageWithURL:[NSURL URLWithString:_detailObj.cover] placeholderImage:[UIImage imageNamed:@"place_holer_750x500"]];
        _carLabel.text = kFormat(@"%@%@",_detailObj.brand, _detailObj.model);
    }];
    kWeakSelf;
    NSDictionary *dic =@{@"orderId":self.orderID,@"token":[[NSUserDefaults standardUserDefaults]objectForKey:access_Token]};
    
    [HttpManage renewMapParameter:dic successBlock:^(NSDictionary *data) {
        [JSFProgressHUD hiddenHUD:self.view];
        _renewMapModel= [[VFRenewMapModel alloc]initWithDic:data];
        _priceLabel.text = kFormat(@"¥%@元/天", _renewMapModel.dayRental);
        [weakSelf calculateAllmoneyDay:@"3"];
...
- (void)calculateAllmoneyDay:(NSString *)days{
    ...
    int allMoney = [_renewMapModel.dayRental intValue] *[days intValue];
    float vipZK = allMoney *[_renewMapModel.vipZk floatValue];
    float payMoney = vipZK;
//    NSString *money = kFormat(@"%d", allMoney);
//    float payMoney = [money floatValue];
    for (NSDictionary *dic in _renewMapModel.level) {
        ...
        if (day >=[model.minDay intValue] && day <=[model.maxDay intValue]) {
            payMoney = payMoney *[model.zk floatValue];
        }
    }
    _allMoneylabel.text = kFormat(@"预估总金额  ¥%.2f", payMoney);
    NSString *contentStr = @"¥";
...
- (void)createView{
    ...
    UIButton *applybutton = [UIButton newButtonWithTitle:@"申请续租"  sel:@selector(applyButtonClcik) target:self cornerRadius:NO];
    applybutton.frame = CGRectMake(0, kScreenH-49, kScreenW, 49);
    [self.view addSubview:applybutton];
}

- (void)applyButtonClcik{
    NSString *useEndTime = [CustomTool toDateChangTimeStr:kFormat(@"%@ %@", _backTime.text,_backClock.text)];
    NSString *token = [[NSUserDefaults standardUserDefaults]objectForKey:access_Token];
    NSDictionary *dic = @{@"endTime":useEndTime,@"orderId":self.orderID,@"token":token,@"remark":_remarkTextField.text};
    kWeakSelf;
    [HttpManage rentalRenewParameter:dic success:^(NSDictionary *data) {
...
//    [carImage sd_setImageWithURL:[NSURL URLWithString:_model.picUrl] placeholderImage:nil];
...
- (void)createRentCarTimeView{
    ...
    _rentDaylabel = [UILabel initWithTitle:@"3天" withFont:kTextSize textColor:kdetailColor];
...
//    NSString *rightClock = [rightStr substringFromIndex:rightStr.length- 5];
    ...
    //获取还车时间
    NSTimeInterval late1=self.endTimeStr.doubleValue;
