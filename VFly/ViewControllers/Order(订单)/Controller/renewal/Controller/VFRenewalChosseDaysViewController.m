//
//  VFRenewalChosseDaysViewController.m
//  LuxuryCar
//
...
@interface VFRenewalChosseDaysViewController ()<UIPickerViewDelegate,UIPickerViewDataSource,UITextFieldDelegate>
...
@implementation VFRenewalChosseDaysViewController
...
//    _dayslabel.text = kFormat(@"%@天", _rentDays);
    ...
    _dayRentLabel = [UILabel initWithTitle:kFormat(@"日租金 ¥%@", _dataModel.dayRental) withFont:kNewTitle textColor:kMainColor];
    ...
    _allRentMoneyLabel = [UILabel initWithTitle:@"" withFont:kTitleBigSize textColor:kdetailColor];
    ...
    [_pickView selectRow:[_rentDays intValue]-1 inComponent:0 animated:NO];
...
- (void)calculateMoney:(NSString *)days{    
    ...
    long allMoney = [_dataModel.dayRental intValue] *[days intValue];
    float vipZK = allMoney *[_dataModel.vipZk floatValue];
    float payMoney = vipZK;
//    float breaksMoney = allMoney *(1-[_dataModel.vipZk floatValue]);
//    NSString *money = [NSString stringWithFormat:@"%ld",allMoney];
//    float payMoney = [money floatValue];
    float breaksMoney = 0;
    for (NSDictionary *dic in _dataModel.level) {
