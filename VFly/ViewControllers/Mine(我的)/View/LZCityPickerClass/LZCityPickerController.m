...
//    _cityPicker = [LZCityPickerView showInView:self.view didSelectWithBlock:^(NSString *address, NSString *province, NSString *city, NSString *area,NSString *cityCode) {
//
//        if (ws.backBlock) {
//            ws.backBlock(address, province, city, area,cityCode);
//        }
//    } cancelBlock:^{
//
...
+ (instancetype)showPickerInViewController:(UIViewController *)vc selectDic:(NSDictionary*)dic selectBlock:(__actionBlock)block {
    ...
    picker.selectDic = dic;
    picker.backBlock = block;
    [vc addChildViewController:picker];
    [vc.view addSubview:picker.view];
    picker.cityPicker = [LZCityPickerView showInView:picker.view selectDic:dic didSelectWithBlock:^(NSString *address, NSString *province, NSString *city, NSString *area, NSString *cityCode) {
        if (picker.backBlock) {
            picker.backBlock(address, province, city, area,cityCode);
        }
    } cancelBlock:^{
